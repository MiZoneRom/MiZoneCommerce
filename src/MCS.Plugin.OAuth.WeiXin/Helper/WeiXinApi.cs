using MCS.Plugin.OAuth.WeiXin.Model;
using System;
...
namespace MCS.Plugin.OAuth.WeiXin.Assistant
{
    public class WeiXinApi
    {
        public static UserInfo GetUserInfo(string code,string appid,string appsecret)
        {
            ...
            if (err.errcode > 0)
            {
                throw new MCS.Core.Plugins.PluginException("微信登录接口GetToken出错: " + err.errmsg);
            }
            if (string.IsNullOrEmpty(tokenResult.access_token))
            {
                throw new MCS.Core.Plugins.PluginException("微信登录接口返回access_Token为空");
            }
            ...
            if (err.errcode>0)
            {
                throw new MCS.Core.Plugins.PluginException("微信登录接口GetUserInfo出错: " + err.errmsg);
            }
