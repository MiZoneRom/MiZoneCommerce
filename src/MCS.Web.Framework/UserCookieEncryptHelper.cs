    ...
    public class UserCookieEncryptHelper
    {
        ...
        public static string Encrypt(long userId, string role, int iTimeOut = 0)
        {
            ...
            try
            {
                ...
                {
                    var strTokenTimeOut = ConfigurationManager.AppSettings["TokenTimeOut"];
                ...
                text = SecureHelper.AESEncrypt(plainText, _userCookieKey);
                text = SecureHelper.EncodeBase64(text);
                return text;
            }
            catch (Exception ex)
            {
                Log.Error(string.Format("加密用户标识Cookie出错", text), ex);
                throw;
            }
        }
		...
		public static long Decrypt(string userIdCookie, string role, bool checkexpire = false)
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(userIdCookie))
                {
                    ...
                    plainText = SecureHelper.AESDecrypt(userIdCookie, _userCookieKey);//解密
            ...
            {
                Log.Error(string.Format("解密用户标识Cookie出错，Cookie密文：{0}", userIdCookie), ex);
            }
        ...
        private static string GetUserCookieKey()
        {
            ...
            {
                settings.UserCookieKey = SecureHelper.MD5(Guid.NewGuid().ToString());
