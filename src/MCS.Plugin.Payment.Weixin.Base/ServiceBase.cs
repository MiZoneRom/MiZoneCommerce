    ...
    public class ServiceBase : PaymentBase<Config>
    {
                ...
                Items = new FormData.FormItem[] { 
                   ...
                   new  FormData.FormItem(){
                     ...
                     IsRequired = true,
                      Type= FormData.FormItemType.text,
                      Value=config.Key,
                      Tips="商户自己申请时，填写自己申请的商户平台的32位密钥；服务商申请时，填写服务商提供的32位密钥"
                   },
                  //AppId
                   new  FormData.FormItem(){
                     DisplayName = "MCHID",
                     ...
                     IsRequired = true,
                      Type= FormData.FormItemType.text,
                      Value=config.MCHID,
                      Tips="商户自己申请时，填写自己申请的微信支付商户号；服务商申请时，填写服务商提供的微信支付商户号"
                   },//安全证书
                   new  FormData.FormItem(){
                     DisplayName = "商户证书",
                     Name = "pkcs12",
                     IsRequired = true,
                      Type= FormData.FormItemType.text,
                      Value=config.pkcs12,
                      Tips="商户自己申请时，填写自己在商户平台下载的证书；服务商申请时，填写服务商提供的证书"
                   },
                    new  FormData.FormItem(){
                     DisplayName = "服务商商户ID",
                     Name = "Main_Mch_ID",
        ...
        public override PaymentInfo EnterprisePay(EnterprisePayPara para)
        {
            ...
            if (e_return.Value == "SUCCESS")
            {
                ...
                {
                    throw new PluginException("付款时,接口返回异常:" + e_errdes.Value);
                }
            }
            else
            {
                throw new PluginException("付款时,接口返回异常:" + e_result.Value);
            }
            return paymentInfo;
        }
    }
