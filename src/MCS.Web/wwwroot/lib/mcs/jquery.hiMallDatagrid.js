(function ($) {
    ...
    function request(target, param) {
        var opts = $.data(target, "mcsDatagrid").options;
        if (param) {
    ...
    function getSelectRows(target) {
        var opts = $.data(target, "mcsDatagrid").options;
        var data = $.data(target, "mcsDatagrid").data;
        var tBody = $(target).find("tbody");
        if (opts.idField) {
            return $.data(target, "mcsDatagrid").selectedRows;
        } else {
            var rowAry = [];
            $("tr.mcsDatagrid-row-selected", tBody).each(function () {
                var row_index = parseInt($(this).attr("mcsDatagrid-row-index"));
    ...
    function selectAll(target) {
        var opts = $.data(target, "mcsDatagrid").options;
        var data = $.data(target, "mcsDatagrid").data;
        var selectRows = $.data(target, "mcsDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        var rows = data.rows;
        selectRows = [];
        $("div.mcsDatagrid-cell-check input[type=checkbox]:enabled", tBody)
            .prop("checked", true).parents('tr').addClass("mcsDatagrid-row-selected");
        for (var i = 0; i < rows.length; i++) {
            if (opts.idField) {
                var row = rows[i];
                if ($("div.mcsDatagrid-cell-check input[type=checkbox]").eq(i).is(':checked')) {
                    selectRows.push(row);
                }
            }
        }
        $.data(target, "mcsDatagrid").selectedRows = selectRows;
    };
    function clearSelectRows(target) {
        ...
        var selectRows = $.data(target, "mcsDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        $("tr", tBody).removeClass("mcsDatagrid-row-selected");
        $("div.mcsDatagrid-cell-check input[type=checkbox]", tBody).prop("checked", false);
    ...
    function selectRow(target, index) {
        ...
        var tr = $("tr[mcsDatagrid-row-index=" + index + "]", tBody);
        if (!tr.hasClass("mcsDatagrid-row-selected")) {
            tr.addClass("mcsDatagrid-row-selected");
            var ck = $("div.mcsDatagrid-cell-check input[type=checkbox]", tr);
    ...
    function unSelectRow(target, index) {
        ...
        var tBody = $(target).find("tbody");
        var tr = $("tr[mcsDatagrid-row-index=" + index + "]", tBody);
        var ck = $("tr[mcsDatagrid-row-index=" + index + "] div.mcsDatagrid-cell-check input[type=checkbox]", tBody);
        tr.removeClass("mcsDatagrid-row-selected");
    ...
    function bindCellsEvents(target) {
        var opts = $.data(target, "mcsDatagrid").options;
        var total = $.data(target, "mcsDatagrid").data.total;
        ...
        $("div.mcsDatagrid-cell-check input[type=checkbox]", $(target)).unbind("click").bind("click", function (e) {
            var findIndex = $(this).parent().parent().parent().attr("mcsDatagrid-row-index");
            if (opts.singleSelect) {
                clearSelections(target);
    ...
    function loadData(target, _data) {
        ...
        $.data(target, "mcsDatagrid").data = _data;
    ...
    //插件构造函数
    $.fn.mcsDatagrid = function (options, param) {
        if (typeof options == "string") {
            return $.fn.mcsDatagrid.methods[options](this, param);
        }

        options = options || {};
        return this.each(function () {
            var state = $.data(this, "mcsDatagrid");
            var opts;
            if (state) {
                opts = $.extend(state.options, options);
                state.options = opts;
            } else {
                opts = $.extend({}, $.fn.mcsDatagrid.defaults, options);
                //console.log(opts);
                $.data(this, "mcsDatagrid", {
                    options: opts,
                    selectedRows: [],
                    data: {
                        total: 0,
    ...
    //插件对外公开的所有方法集合
    $.fn.mcsDatagrid.methods = {
        options: function (jq) {
            var opts = $.data(jq[0], "mcsDatagrid").options;
            return opts;
        },
        data: function (jq) {
            var data = $.data(jq[0], "mcsDatagrid").data;
            return data;
        },
        reload: function (jq, param) {
            return jq.each(function () {
                if (!param.pageNumber) param.pageNumber = 1;
                $.data(this, "mcsDatagrid").options.pageNumber = param.pageNumber;
                request(this, param);
            });
        },
        ...
        loading: function (jq) {
            return jq.each(function () {
                var opts = $.data(this, "mcsDatagrid").options;
        ...
        getRows: function (jq) {
            return $.data(jq[0], "mcsDatagrid").data.rows;
        },
    ...
    var renderView = {
        render: function (target, container) {
            var opts = $.data(target, "mcsDatagrid").options;
            var rows = $.data(target, "mcsDatagrid").data.rows;
            var closFileds = $(target).mcsDatagrid("getColumnFields");
            ...
            if (rows == null || rows.length <= 0) {
                ...
                var rowData = {};
                for (var i = 0; i < rows.length; i++) {
                    var cls = (i % 2 && opts.striped) ? "class=\"mcsDatagrid-row-alt\"" : "";
                    ...
                    if (opts.childField && rows[i][opts.childField].length > 0) {
                        for (var j = 0; j < rows[i][opts.childField].length ; j++) {
                            ...
                            htmlAry.push("<tr mcsDatagrid-row-index=\"" + i + "\" " + cls + " " + strStyle + " ");
        ...
        renderHead: function (target, container) {
            ...
            for (var i = 0; i < closFileds.length; i++) {
                ...
                var content = "",
                	...
                	col = $(target).mcsDatagrid("getColumnOption", findFiled),
                	sort = '',
                	sortHtml = '',
        ...
        renderRow: function (target, fileds, rowIndex, rowData) {
            ...
            for (var i = 0; i < fileds.length; i++) {
                ...
                var col = $(target).mcsDatagrid("getColumnOption", findFiled);
                if (col) {
                    var className = "";
                    ...
                    if (col.checkbox) {
                        cc.push(" class=\"mcsDatagrid-cell-check ");
                    } else {
                        cc.push(" class=\"mcsDatagrid-cell ");
                    }
                    cc.push("\">");
                    if (col.checkbox) {
                        if (col.formatter) {
