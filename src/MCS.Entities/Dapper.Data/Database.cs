    ...
    public class Database : IDisposable
    {
        ...
        public Database(string connectionStringName)
        {
            ...
            if (ConfigurationManager.ConnectionStrings[connectionStringName] != null)
            {
                if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings[$"{connectionStringName}:ProviderName"]))
                    providerName = ConfigurationManager.AppSettings[$"{connectionStringName}:ProviderName"];
            }
            else
            {
                throw new InvalidOperationException($"在配置文件中没有找到名为“{connectionStringName}”的数据源连接设置");
            }
        ...
        public virtual void OnExecutedCommand(IDbCommand cmd)
        {
            ...
            if (time.TotalMilliseconds > 3000)
                WMS.Core.Log.Info(string.Format("超时SQL:{0}毫秒,{1}", time.TotalMilliseconds, LastCommand));
        ...
        public void WriteLog()
        {
            try
            {
                Log.Debug(string.Format("时间: {0}\t数据库类型:{1}{2}{3}{2}{2}", DateTime.Now.ToString(), this._dbType.GetType().Name, Environment.NewLine, this.LastCommand));
            }
            catch
            {
